#!/usr/bin/env bb
;; kizuna — bb-native test runner (Clojure / babashka; no shell). Auto-generated
...
;;
;;   bb 20-actors/kizuna/run_tests.clj      ; run from anywhere
(require '[babashka.classpath :as cp]
         '[babashka.fs :as fs]
         '[clojure.test :as t])

;; this file is 20-actors/kizuna/run_tests.clj -> classpath root is its grandparent (20-actors/)
(cp/add-classpath (str (fs/parent (fs/parent (fs/absolutize *file*)))))

(def suites
  '[kizuna.tests.test-kizuna])

(apply require suites)

(let [{:keys [fail error]} (apply t/run-tests suites)]
  (if (zero? (+ fail error))
    (println "-- kizuna: ALL suites green --")
    (do (println "-- kizuna: FAILURES above --")
        (System/exit 1))))
