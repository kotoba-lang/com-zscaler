(ns kizuna.methods.social
  "social.cljc — kizuna DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from kizuna's own
...
(defn draft-observation-post
  "A dry-run observation post over kizuna's own domain (kizuna actor domain)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "kizuna R0: live social posting is Council Lv6+ + operator + "
