(ns zengin_system.main
  "Kotodama WASM entrypoint for the Zengin_system clean-room actor (L4) — Clojure port.
  ...
  facts (`zengin_system.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "zengin_system")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"zengin_system.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "zengin_system-compat" :tier tier :entities entities} 200])
...
(defn start! [] :zengin_system-compat/ready)
