(ns hinagata.methods.social
  "social.cljc — hinagata DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from hinagata's own
...
(defn draft-observation-post
  "A dry-run observation post over hinagata's own domain (Legal-document-template commons (法律文書雛形) — weaves fair, neutral, openly-licensed legal-document templates (contracts / agreements /…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "hinagata R0: live social posting is Council Lv6+ + operator + "
