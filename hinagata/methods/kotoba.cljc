#!/usr/bin/env bb
;; hinagata 雛形 — legal-template-commons DATOM-LOG writer (content-addressed commit-DAG).
(ns hinagata.methods.kotoba
  "kotoba.cljc — hinagata 雛形 append-only TEMPLATE-COMMONS LEDGER writer
  (ADR-2606111954, on ADR-2605262130 + ADR-2605312345; same content-addressed
  commit-DAG machinery as the tate/ugachi/busshi family — the datomic-isomorphic
  canonical-state layer hinagata previously only EMITTED (datom_emit) but never
  PERSISTED).

  Persists hinagata's GROUND datoms — the template/clause/statute/jurisdiction/
  concept/license NODES and their :en/* 縁 (all :authoritative or :representative
  PUBLIC reference facts, G1) — to a local append-only EDN log of content-addressed
  transactions. DERIVED state (groundedness / reusability / statute-pull, the
  integral of incident 縁 computed on READ, N1/G2) is NEVER stored: the ledger
  holds only durable ground facts, so a seed edit appends but a re-read does not.
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; hinagata 雛形 — TEMPLATE-COMMONS LEDGER (append-only, "
                        "content-addressed EAVT commit-DAG of the public template/clause/statute "
                        "graph). Generated; DO NOT hand-edit. A COMMONS record, never advice and "
                        "never a verdict. ADR-2606111954 / ADR-2605312345.\n")))
