(ns chigiri.methods.social
  "social.cljc — 契 — Covenant (legal procedure substrate) DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from chigiri's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "契 — Covenant (legal procedure substrate) が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over chigiri's own domain (Non-profit religious-corp legal procedure substrate. Procedural template + on-chain attestation + routing substrate for the 任意団体 internal…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "chigiri R0: live social posting is Council Lv6+ + operator + "
