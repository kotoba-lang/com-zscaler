#!/usr/bin/env bb
;; chigiri 契 — legal-aid referral-registry DATOM-LOG writer (content-addressed commit-DAG).
(ns chigiri.methods.kotoba
  "kotoba.cljc — chigiri 契 append-only REFERRAL-REGISTRY LEDGER writer
  (ADR-2605262700, on ADR-2605262130 + ADR-2605312345; same content-addressed
  commit-DAG machinery as the tate/hinagata/ugachi/busshi family).

  Persists chigiri's GROUND datoms — the legal-aid REFERRAL nodes + their disclosed
  wayfinding attributes — to a local append-only EDN log of content-addressed
  transactions. Each tx CID = 'b' + sha256-hex over the canonical JSON
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; chigiri 契 — REFERRAL-REGISTRY LEDGER (append-only, content-addressed "
                        "EAVT commit-DAG of the worldwide legal-aid referral registry). Generated; "
                        "DO NOT hand-edit. Routing to LICENSED human counsel, never advice. "
                        "ADR-2605262700 / ADR-2605312345.\n")))
