...
describe("Oil Refining Actor Manifest", () => {
  it("@context valid", () => { expect(m["@context"]).toBe("https://etzhayyim.com/ns/actor/v1"); });
  it("DID valid", () => { expect(m["@id"]).toBe("did:web:oil-refining.etzhayyim.com"); });
  it("runtime", () => { expect(m.runtime).toBe("k8s-langserver"); });
  it("nanoid", () => { expect(m.nanoid).toBe("01lr3f1n"); });
  ...
  it("cron report has 5 steps", () => {
    ...
    expect(cron.steps[2].id).toBe("outages");
  });
  it("subscribes to oilShipping.cargo", () => {
    const sub = m.pipelines.find((p: any) => p.trigger.type === "subscribeRepos");
    expect(sub.trigger.collections).toContain("com.etzhayyim.apps.oilShipping.cargo");
  });
  it("xrpc covers refinery, units, yield pressure, health", () => {
    ...
    expect(nsids).toContain("com.etzhayyim.apps.oilRefining.registry.getRefinery");
    expect(nsids).toContain("com.etzhayyim.apps.oilRefining.registry.listRefineries");
    expect(nsids).toContain("com.etzhayyim.apps.oilRefining.registry.listUnits");
    expect(nsids).toContain("com.etzhayyim.apps.oilRefining.analytics.getYieldPressure");
    expect(nsids).toContain("com.etzhayyim.apps.oilRefining.health");
  });
