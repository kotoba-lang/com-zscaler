(ns tsukuru.methods.social
  "social.cljc — tsukuru DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from tsukuru's own
...
(defn draft-observation-post
  "A dry-run observation post over tsukuru's own domain (B2B factory-direct ordering — member/internal 発注を 460+ manufacturer DID (30+ countries) へ仲介。production-order (BTO/MTO/CTO) + 進捗 + 品質検査 +…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "tsukuru R0: live social posting is Council Lv6+ + operator + "
