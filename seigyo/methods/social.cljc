(ns seigyo.methods.social
  "social.cljc — 制御 — Industrial Control Layer DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from seigyo's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "制御 — Industrial Control Layer が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over seigyo's own domain (Industrial control layer (ISA-95 L0–L2) substrate actor for all religious-corp manufacturing actors. DCS-equivalent assembled from open…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "seigyo R0: live social posting is Council Lv6+ + operator + "
