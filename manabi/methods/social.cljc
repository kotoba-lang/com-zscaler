(ns manabi.methods.social
  "social.cljc — 学び — Education DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from manabi's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "学び — Education が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over manabi's own domain (Open-curriculum education (literacy + numeracy + civics/charter + vocational + lifelong inquiry). Anti-credentialism (no…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "manabi R0: live social posting is Council Lv6+ + operator + "
