#!/usr/bin/env bash
# manabi 学び — kotoba deploy
# ADR-2605261045 · migration plan Phase 3
#
# Ingests schema-shaped seed datoms (modules + learner sessions + skill attestations)
# into a running kotoba node and (optionally) builds the langgraph WASM actors
# (5-handler graph). Writes to the canonical Datom journal require an authorized
# operator session token (no-server-key posture, G15). Without KOTOBA_TOKEN the
# ingest is a dry-run. Replaces the legacy Python cell tracking path (G6).
#
# Usage:
...
GRAPH="${MANABI_GRAPH:-com.etzhayyim.manabi}"
ACTOR_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

echo "==> manabi kotoba deploy → ${KOTOBA_URL} (graph ${GRAPH})"
...
# 0.5 module registry projection (representative R0; live curriculum library is G4-gated)
echo "--> curriculum registry ingest (:module/* datoms)"
