(ns inochi.methods.test-datom-emit
  "inochi 命 — Datom-emit tests (ADR-2606073000). 1:1 port of the two datom_emit-dependent
...
(deftest test-datom-emit-ground-and-transient
  (let [{:keys [nodes edges]} (datom-emit/load-file* seed)
    ...
    (is (str/includes? out ":taxon/iucn") "node attribute datoms missing")
    ...
    (is (str/includes? out ":bond/restoration-priority"))
