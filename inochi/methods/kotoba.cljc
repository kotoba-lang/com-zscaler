#!/usr/bin/env bb
;; inochi 命 — biosphere-graph DATOM-LOG writer (content-addressed commit-DAG).
(ns inochi.methods.kotoba
  "kotoba.cljc — inochi 命 append-only BIOSPHERE LEDGER writer
  (ADR-2606073000, on ADR-2605262130 + ADR-2605312345; same content-addressed
  commit-DAG machinery as the tate/hinagata/chigiri/kadode/kaiyaku/ugachi/busshi
  family — the datomic-isomorphic canonical-state layer inochi previously only
  EMITTED (datom_emit) but never PERSISTED).

  Persists inochi's GROUND datoms — the organism/taxon/ecosystem NODES and the
  pressure :en/* 縁 (PUBLIC reference biosphere data, G1) — to a local append-only
  EDN log of content-addressed transactions. DERIVED state (restoration-priority /
  dependency-fragility / pressure-imposed, the integral of incident 縁 computed on
  READ, N1/G2) is NEVER stored.
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; inochi 命 — BIOSPHERE LEDGER (append-only, content-addressed EAVT "
                        "commit-DAG of the living-world organism/ecosystem/pressure graph). Generated; "
                        "DO NOT hand-edit. A RESTORATION map, never a target-list (no coordinates). "
                        "ADR-2606073000 / ADR-2605312345.\n")))
