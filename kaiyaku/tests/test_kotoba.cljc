#!/usr/bin/env bb
;; kaiyaku 解約 — enkiri-ledger persistence + heartbeat tests.
;; Run:  bb --classpath 20-actors 20-actors/kaiyaku/tests/test_kotoba.cljc
...
(defn- tmp [] (str (System/getProperty "java.io.tmpdir") "/kaiyaku-ledger-test-" (gensym) ".edn"))
(defn- d1 [] [(k/add "svc:streaming" ":svc/kind" ":subscription")
              (k/add "svc:streaming" ":svc/notice-days" 30)])
(defn- d2 [] [(k/add "en:000" ":en/kind" ":subscribes")
              (k/add "en:000" ":en/monthly-cost-jpy" 1980.0)])
...
(deftest float-monthly-cost-roundtrips
  (let [p (tmp)]
    (try
      (k/append-tx (k/make-tx (d2) "t1" "as1" "") p)
      (is (= (d2) (get (first (k/read-log p)) ":tx/datoms")) "float :en/monthly-cost-jpy round-trips")
      (finally (io/delete-file p true)))))
  ...
  (let [p (tmp)]
    (try
      (let [c1 (k/append-tx (k/make-tx (d1) "t1" "as1" "") p)]
        ...
        (let [corrupted (str/replace (slurp p) ":subscribes" ":depends-on")]
          (spit p corrupted)
...
;; ── heartbeat (autorun) over the REAL committed 縁-ledger seed ────────────────

(deftest beat-ground-only-and-real-seed
  (let [ds (auto/ground-datoms)]
    (is (seq ds) "ground datoms are produced from the real committed 縁-ledger seed")
    (is (every? #(= ":db/add" (first %)) ds) "every datom is an :add (EAVT op)")
    (is (some (fn [[_ _ a]] (= ":en/from" a)) ds) "縁 (ties) are persisted (graph not just nodes)")
    (is (not-any? (fn [[_ _ a]] (str/starts-with? (str a) ":bond/")) ds)
        "GROUND only — derived :bond/* readouts not persisted (G2)")
    (is (not-any? (fn [[_ _ a]] (str/starts-with? (str a) ":enkiri/")) ds)
        "GROUND only — :enkiri/burden / recommendation are derived, not persisted (G2)")))
  ...
  (let [p (tmp)]
    (try
      (let [r1 (auto/beat {:tx-id "t1" :as-of "a1" :log-path p})
        ...
        (is (pos? (:count r1)))
        (is (not (:appended r2)) "second beat over identical 縁-ledger is a NO-OP")
...
#?(:clj
   (when (= *file* (System/getProperty "babashka.file"))
     (let [{:keys [fail error]} (run-tests 'kaiyaku.tests.test-kotoba)]
