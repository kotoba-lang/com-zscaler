(ns kaiyaku.methods.social
  "social.cljc — kaiyaku DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from kaiyaku's own
...
(defn draft-observation-post
  "A dry-run observation post over kaiyaku's own domain (縁切り (tie-severance) EXECUTOR for the member's OWN service ties — the missing last leg of the organizer subscription-discovery pipeline…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "kaiyaku R0: live social posting is Council Lv6+ + operator + "
