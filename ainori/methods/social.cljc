(ns ainori.methods.social
  "social.cljc — ainori DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from ainori's own
...
(defn draft-observation-post
  "A dry-run observation post over ainori's own domain (Uber-inverse pooled passenger-mobility commons — members already travelling offer seats; riders cost-share (fuel/wear only, no margin); the…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "ainori R0: live social posting is Council Lv6+ + operator + "
