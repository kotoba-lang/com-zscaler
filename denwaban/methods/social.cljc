(ns denwaban.methods.social
  "social.cljc — denwaban DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from denwaban's own
...
(defn draft-observation-post
  "A dry-run observation post over denwaban's own domain (Voice receptionist — answers an inbound call (PSTN/SIP via twilio-compat, or a WebRTC soft-phone over kotoba-net), converses by voice…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "denwaban R0: live social posting is Council Lv6+ + operator + "
