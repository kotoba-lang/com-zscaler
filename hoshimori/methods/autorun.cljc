#!/usr/bin/env bb
;; hoshimori 星守 — autonomous heartbeat: load the orbit graph → append GROUND datoms.
(ns hoshimori.methods.autorun
  "autorun.cljc — hoshimori 星守 deterministic heartbeat (ADR-2606073600, inochi/asobi pattern).

  One beat: load the orbital-stewardship seed graph, build the GROUND EAVT datoms (shell/
  operator/hazard/service NODES + their congestion :en/* 縁), and APPEND them as ONE content-
  addressed transaction to the append-only ORBITAL-STEWARDSHIP LEDGER (kotoba.cljc). prev-cid
  chaining keeps the ledger tamper-evident + resume-safe.

  GROUND ONLY: congestion-concentration / stewardship-buffer / dependency-fragility /
  congestion-imposed are DERIVED (computed on read, N1/G2) and are NEVER persisted.
  Deterministic by construction: the caller supplies tx-id + as-of (no wall clock); nodes
...
#?(:clj
   (defn default-seed-path []
     (str (io/file here-dir "data" "seed-orbit-graph.kotoba.edn"))))
...
(defn beat
  "Run one heartbeat. opts:
     ...
     :log-path orbital-stewardship-ledger path (required)
...
#?(:clj
   (defn -main [& args]
     (let [log-path (or (first args)
                        (str (io/file here-dir "data" "persisted" "hoshimori.orbital-stewardship.kotoba.edn")))
           r (beat {:tx-id "hoshimori-beat-manual" :as-of "manual" :log-path log-path})]
       (println (str "orbital-stewardship ledger head=" (:head r)
                     " datoms=" (:count r)
                     " appended=" (:appended r)
