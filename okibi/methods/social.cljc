(ns okibi.methods.social
  "social.cljc — okibi DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from okibi's own
...
(defn draft-observation-post
  "A dry-run observation post over okibi's own domain (Thermal Matching Market — the waste-heat leg of the Energy Order Protocol suite. Heat is local (distance-weak), so okibi matches waste-heat…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "okibi R0: live social posting is Council Lv6+ + operator + "
