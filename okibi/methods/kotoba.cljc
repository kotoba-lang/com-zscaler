#!/usr/bin/env bb
;; 燠 okibi — thermal-matching-ledger kotoba Datom-log writer (content-addressed).
(ns okibi.methods.kotoba
  "kotoba.cljc — 燠 okibi append-only THERMAL MATCHING LEDGER writer (on
  ADR-2605262130 + ADR-2605312345; same content-addressed commit-DAG machinery as
  the mio/tawami family).

  Persists the thermal MATCHES to a local append-only EDN log of content-addressed
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; 燠 okibi — THERMAL MATCHING LEDGER (append-only, content-addressed "
                        "EAVT commit-DAG of thermal matches). Generated; DO NOT hand-edit. "
                        "A matching map, never a dispatch order.\n")))
