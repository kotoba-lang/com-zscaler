(ns yadori.methods.social
  "social.cljc — yadori DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from yadori's own
...
(defn draft-observation-post
  "A dry-run observation post over yadori's own domain (DNS-availability check + domain acquisition (宿り = taking up dwelling). RDAP/EPP availability lookup + Murakumo-only name suggestion +…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "yadori R0: live social posting is Council Lv6+ + operator + "
