#!/usr/bin/env bb
;; edn-datomize.bb — EDN → Datomic/Datascript tx-data 変換ツール（hataori 用に adapt）。
;; 出自: com-junkawasaki/root superproject の manifest/edn-datomize.bb (Phase 1/2 実装)、
;; 同型 port は 20-actors/sanae/edn-datomize.bb (Phase 4, 同じ labor-liberation wave) を参照。
;; schema-path はこのディレクトリ（20-actors/hataori/）直下（repo-root に manifest/ 規約が無いため）。
;;
;; 使い方:
...
(def root (str (io/file (System/getProperty "user.dir") "20-actors" "hataori")))
