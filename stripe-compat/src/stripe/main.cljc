(ns stripe.main
  "Kotodama WASM entrypoint for the Stripe clean-room actor (L4) — Clojure port.
  ...
  facts (`stripe.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "stripe")
(def tier "L4")
...
(def entity-specs
  [{:entity "Customer"       :plural "customers"       :id-prefix "stripe_cus"
    :fields [:email :name :phone :balance :currency]                :required [:email :name]
    :coerce {:balance :int}                            :refs {}}
   {:entity "PaymentIntent"  :plural "paymentintents"  :id-prefix "stripe_pay"
    :fields [:customer :amount :currency :status :description]      :required [:customer :amount]
    :coerce {:amount :int}                             :refs {}}
   {:entity "Charge"         :plural "charges"         :id-prefix "stripe_cha"
    :fields [:customer :paymentIntent :amount :currency :status]    :required [:customer :paymentIntent]
    :coerce {:amount :int}                             :refs {}}
   {:entity "Refund"         :plural "refunds"         :id-prefix "stripe_ref"
    :fields [:charge :amount :reason :status]                       :required [:charge :amount]
    :coerce {:amount :int}                             :refs {}}
   {:entity "Invoice"        :plural "invoices"        :id-prefix "stripe_inv"
    :fields [:customer :amountDue :currency :status]                :required [:customer :amountDue]
    :coerce {:amountDue :int}                          :refs {}}
   {:entity "Subscription"   :plural "subscriptions"   :id-prefix "stripe_sub"
    :fields [:customer :priceId :status :currentPeriodEnd]          :required [:customer :status]
    :coerce {}                                         :refs {:priceId "Price"}}
   {:entity "Product"        :plural "products"        :id-prefix "stripe_pro"
    :fields [:name :description :active]                            :required [:name :description]
    :coerce {:active :bool}                            :refs {}}
   {:entity "Price"          :plural "prices"          :id-prefix "stripe_pri"
    :fields [:product :unitAmount :currency :recurringInterval]     :required [:product :unitAmount]
    :coerce {:unitAmount :int}                         :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"stripe.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "stripe-compat" :tier tier :entities entities} 200])
...
(defn start! [] :stripe-compat/ready)
