(ns yardi_voyager.main
  "Kotodama WASM entrypoint for the Yardi_voyager clean-room actor (L4) — Clojure port.
  ...
  facts (`yardi_voyager.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "yardi_voyager")
(def tier "L4")
...
(def entity-specs
  [{:entity "Listing"   :plural "listings"   :id-prefix "yardivoy_lis"
   ...
   {:entity "Tenant"   :plural "tenants"   :id-prefix "yardivoy_ten"
...
(defn emit-facts
  "EAVT facts for one record: {\"yardi_voyager.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "yardi_voyager-compat" :tier tier :entities entities} 200])
...
(defn start! [] :yardi_voyager-compat/ready)
