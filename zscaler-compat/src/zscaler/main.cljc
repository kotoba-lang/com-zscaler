(ns zscaler.main
  "Kotodama WASM entrypoint for the Zscaler clean-room actor (L4) — Clojure port.
  ...
  facts (`zscaler.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "zscaler")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"zscaler.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "zscaler-compat" :tier tier :entities entities} 200])
...
(defn start! [] :zscaler-compat/ready)
