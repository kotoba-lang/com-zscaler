...
set -euo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"    # 20-actors/suki
REPO="$(dirname "$(dirname "$HERE")")"   # repo root (or worktree root)
cd "$REPO"

if ! command -v bb >/dev/null 2>&1; then
  echo "── suki: babashka (bb) not found — skipping ──"
  exit 0
fi

bb --classpath 20-actors -e '
(require (quote [clojure.test :as t]))
(require (quote suki.methods.test-charter-gates))
(let [r (t/run-tests (quote suki.methods.test-charter-gates))]
  (println "── suki:" (:test r) "tests /" (:pass r) "assertions green,"
