(ns zoho.main
  "Kotodama WASM entrypoint for the Zoho clean-room actor (L4) — Clojure port.
  ...
  facts (`zoho.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"zoho.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "zoho-compat" :tier tier :entities entities} 200])
