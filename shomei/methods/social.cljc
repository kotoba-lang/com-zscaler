(ns shomei.methods.social
  "social.cljc — 証明 — Believer Identity Binding + Proof-of-Personhood DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from shomei's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "証明 — Believer Identity Binding + Proof-of-Personhood が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over shomei's own domain (Binds MULTIPLE external identities (government IDs, SNS accounts, crypto wallets) and existing etzhayyim substrate commitments to a single…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "shomei R0: live social posting is Council Lv6+ + operator + "
