...
;;
;; このコピーは etzhayyim/root 20-actors/kiyome 用に root/schema-path のみ調整
;; （sparse worktree では `git rev-parse --show-toplevel` はワークツリー全体の
;; ルートを返してしまい 20-actors/kiyome ではないため、このスクリプト自身の
;; 置き場所＝20-actors/kiyome/ を root に固定し、schema.edn もここに置く。
;; manifest/ ディレクトリ規約が無いこの actor-fanout コンテキスト向け）。
;; 他のロジックは com-junkawasaki/root の manifest/edn-datomize.bb と同一。
...
(def root (str/trim (:out (shell/sh "pwd"))))
