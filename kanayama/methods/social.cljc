(ns kanayama.methods.social
  "social.cljc — 金山 — Circular Metallurgy DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from kanayama's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "金山 — Circular Metallurgy が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over kanayama's own domain (Closed-loop material recovery / circular metallurgy Tier-B actor. Wave 1 reference = closed-loop aluminum used beverage container (UBC)…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "kanayama R0: live social posting is Council Lv6+ + operator + "
