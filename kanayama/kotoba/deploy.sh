#!/usr/bin/env bash
# kanayama 金 — kotoba deploy
# ADR-2605252400 · migration plan Phase 3
#
# Ingests schema-shaped seed datoms (UBC intake + full recycling process) into a
...
GRAPH="${KANAYAMA_GRAPH:-com.etzhayyim.kanayama}"
ACTOR_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

echo "==> kanayama kotoba deploy → ${KOTOBA_URL} (graph ${GRAPH})"
