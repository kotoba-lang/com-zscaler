(ns utsushie.methods.social
  "social.cljc — 写し絵 — News→Video Medium (kawaraban's projected-image sibling) DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from utsushie's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "写し絵 — News→Video Medium (kawaraban's projected-image sibling) が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over utsushie's own domain (A NEWS→VIDEO MEDIUM. Turns a kawaraban :article/面 into a short narrated, multilingual video that LINKS OUT — the 映像 (moving-image) sibling…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "utsushie R0: live social posting is Council Lv6+ + operator + "
