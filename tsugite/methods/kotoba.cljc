#!/usr/bin/env bb
;; tsugite 継ぎ手 — peoples-continuity graph DATOM-LOG writer (content-addressed commit-DAG).
(ns tsugite.methods.kotoba
  "kotoba.cljc — tsugite 継ぎ手 append-only PEOPLES-CONTINUITY LEDGER writer
  (ADR-2606073800, on ADR-2605262130 + ADR-2605312345; same content-addressed
  commit-DAG machinery as the inochi/asobi/hokorobi/tate/ugachi/busshi family — the
  datomic-isomorphic canonical-state layer tsugite previously only EMITTED
  (datom_emit) but never PERSISTED).

  Persists tsugite's GROUND datoms — the human-COLLECTIVE / language / pressure / haven
  NODES and the peril :en/* 縁 (PUBLIC aggregate data, G1) — to a local append-only EDN
  log of content-addressed transactions. DERIVED state (continuity-need / protection /
  pressure readouts, the integral of incident 縁 computed on READ, N1/G2) is NEVER stored.
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; tsugite 継ぎ手 — PEOPLES-CONTINUITY LEDGER (append-only, content-addressed "
                        "EAVT commit-DAG of the human-collective/language/peril graph). Generated; DO "
                        "NOT hand-edit. A CONTINUITY map at AGGREGATE scale, never person-tracking. "
                        "ADR-2606073800 / ADR-2605312345.\n")))
