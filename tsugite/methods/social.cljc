(ns tsugite.methods.social
  "social.cljc — tsugite DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from tsugite's own
...
(defn draft-observation-post
  "A dry-run observation post over tsugite's own domain (World peoples-continuity (民の存続) mirror — weaves human COLLECTIVES (migration/refugee/stateless populations, indigenous peoples, language…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "tsugite R0: live social posting is Council Lv6+ + operator + "
