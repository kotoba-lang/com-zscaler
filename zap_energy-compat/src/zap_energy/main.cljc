(ns zap_energy.main
  "Kotodama WASM entrypoint for the Zap_energy clean-room actor (L4) — Clojure port.
  ...
  facts (`zap_energy.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "zap_energy")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"zap_energy.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "zap_energy-compat" :tier tier :entities entities} 200])
...
(defn start! [] :zap_energy-compat/ready)
