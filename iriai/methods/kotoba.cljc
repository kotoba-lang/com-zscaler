...
(ns iriai.methods.kotoba
  ...
  Persists the lifeline-commons INFRA verdicts + FUNDING proposals + GOVERNANCE
  decisions to a local append-only EDN log of content-addressed transactions — the
  'commons ledger'. Each tx CID = 'b' + sha256-hex over the canonical JSON
  ...
  No-server-key (G6): this writer holds no key and performs NO network I/O — it only
