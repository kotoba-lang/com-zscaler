(ns zomato.main
  "Kotodama WASM entrypoint for the Zomato clean-room actor (L4) — Clojure port.
  ...
  facts (`zomato.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "zomato")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"zomato.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "zomato-compat" :tier tier :entities entities} 200])
...
(defn start! [] :zomato-compat/ready)
