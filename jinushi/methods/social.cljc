(ns jinushi.methods.social
  "social.cljc — 地主 — World Land-Ownership Acquisition Mirror DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from jinushi's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "地主 — World Land-Ownership Acquisition Mirror が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over jinushi's own domain (jinushi actor domain)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "jinushi R0: live social posting is Council Lv6+ + operator + "
