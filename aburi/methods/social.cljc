(ns aburi.methods.social
  "social.cljc — aburi DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from aburi's own
...
(defn draft-observation-post
  "A dry-run observation post over aburi's own domain (Personal-tracking-exposure observatory (member-side, own-data) — answers 'when I accept the ToS / grant a permission on Google / Facebook /…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "aburi R0: live social posting is Council Lv6+ + operator + "
