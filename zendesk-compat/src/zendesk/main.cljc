(ns zendesk.main
  "Kotodama WASM entrypoint for the Zendesk clean-room actor (L4) — Clojure port.
  ...
  facts (`zendesk.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "zendesk")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"zendesk.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "zendesk-compat" :tier tier :entities entities} 200])
...
(defn start! [] :zendesk-compat/ready)
