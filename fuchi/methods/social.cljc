(ns fuchi.methods.social
  "social.cljc — 扶持 — Maintainer Sustenance Allocator (investment-fund inverse) DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from fuchi's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "扶持 — Maintainer Sustenance Allocator (investment-fund inverse) が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over fuchi's own domain (Mission-aligned maintainer sustenance allocator — the charter-clean INVERSE of a business investment fund. Where a VC fund invests capital…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "fuchi R0: live social posting is Council Lv6+ + operator + "
