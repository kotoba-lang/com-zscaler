(ns wadachi.methods.social
  "social.cljc — wadachi DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from wadachi's own
...
(defn draft-observation-post
  "A dry-run observation post over wadachi's own domain (Autonomous-mobility R&D (SAE L4 ceiling) — ground autonomous vehicle orchestration for on-site + inter-site ground transport.)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "wadachi R0: live social posting is Council Lv6+ + operator + "
