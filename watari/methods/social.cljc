(ns watari.methods.social
  "social.cljc — 渡り (わたり) DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from watari's own
...
(defn draft-observation-post
  "A dry-run observation post over watari's own domain (World live moving-craft knowledge graph — ingests the LIVE positions of public, transponder-broadcasting craft (ships via AIS, aircraft via…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "watari R0: live social posting is Council Lv6+ + operator + "
