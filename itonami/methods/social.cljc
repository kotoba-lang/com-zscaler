(ns itonami.methods.social
  "social.cljc — itonami DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from itonami's own
...
(defn draft-observation-post
  "A dry-run observation post over itonami's own domain (Factory-operations agent — the charter-clean inversion of an 'AI Factory Brain' / NVIDIA Factory Operations Blueprint (FOX). Observes the…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "itonami R0: live social posting is Council Lv6+ + operator + "
