(ns zipline.main
  "Kotodama WASM entrypoint for the Zipline clean-room actor (L4) — Clojure port.
  ...
  facts (`zipline.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "zipline")
(def tier "L4")
...
(def entity-specs
  [{:entity "Drone"     :plural "drones"     :id-prefix "zipline_dro"
   ...
   {:entity "Telemetry" :plural "telemetries" :id-prefix "zipline_tel"
   ...
   {:entity "Payload"   :plural "payloads"   :id-prefix "zipline_pay"
    :fields [:droneId :type :weightKg]                          :required [:type :weightKg]
    :coerce {:weightKg :float}              :refs {:droneId "Drone"}}
   {:entity "Swarm"     :plural "swarms"     :id-prefix "zipline_swa"
...
(defn emit-facts
  "EAVT facts for one record: {\"zipline.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "zipline-compat" :tier tier :entities entities} 200])
...
(defn start! [] :zipline-compat/ready)
