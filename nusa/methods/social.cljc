(ns nusa.methods.social
  "social.cljc — nusa DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from nusa's own
...
(defn draft-observation-post
  "A dry-run observation post over nusa's own domain (Ritual + industrial HEMP (麻) heritage knowledge graph + low-THC cultivation-revival design (幣 = ōnusa, the hemp purification wand).…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "nusa R0: live social posting is Council Lv6+ + operator + "
