(ns sarutahiko.methods.social
  "social.cljc — 猿田彦 — Heavy Truck Manufacturing DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from sarutahiko's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "猿田彦 — Heavy Truck Manufacturing が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over sarutahiko's own domain (Heavy-truck manufacturing Tier-B actor. Manufacturing-side counterpart of wadachi (operator-side autonomous mobility, ADR-2605242000). Wave…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "sarutahiko R0: live social posting is Council Lv6+ + operator + "
