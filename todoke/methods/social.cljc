(ns todoke.methods.social
  "social.cljc — todoke DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from todoke's own
...
(defn draft-observation-post
  "A dry-run observation post over todoke's own domain (Last-mile ('one-mile') autonomous delivery — curb-to-door small-payload transport that closes the gap between wadachi inter-site ground…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "todoke R0: live social posting is Council Lv6+ + operator + "
