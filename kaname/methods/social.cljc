(ns kaname.methods.social
  "social.cljc — 要 — Cross-Domain System-of-Systems Leverage Synthesizer DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from kaname's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "要 — Cross-Domain System-of-Systems Leverage Synthesizer が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over kaname's own domain (Non-profit religious-corp META synthesis actor — the system-of-systems (SoS) layer above the power-mirror lineage (tsumugi / keizu / kabuto…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "kaname R0: live social posting is Council Lv6+ + operator + "
