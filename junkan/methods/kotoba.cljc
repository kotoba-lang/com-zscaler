#!/usr/bin/env bb
;; junkan 循環 — findings-ledger kotoba Datom-log writer (content-addressed).
(ns junkan.methods.kotoba
  "kotoba.cljc — junkan 循環 append-only FINDINGS LEDGER writer
  (ADR-2605290927, on ADR-2605262130 + ADR-2605312345; same content-addressed
  commit-DAG machinery as the ugachi/meisai family).

  Persists the governance-asymmetry FINDINGS (instrument facts + derived stock
  regimes + loop hypotheses) to a local append-only EDN log of content-addressed
  transactions — junkan's only output is structured, append-only findings (the
  ADR's defining property). Each tx CID = 'b' + sha256-hex over the canonical JSON
  ...
  G4 ANALYSIS-ONLY: this writer holds NO key and performs NO network I/O — it only
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; junkan 循環 — FINDINGS LEDGER (append-only, content-addressed "
                        "EAVT commit-DAG of governance-asymmetry findings). Generated; DO NOT "
                        "hand-edit. A resilience/leverage MAP, never a target-list. ADR-2605290927.\n")))
