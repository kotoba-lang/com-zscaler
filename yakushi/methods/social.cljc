(ns yakushi.methods.social
  "social.cljc — 薬師 (yakushi) DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from yakushi's own
...
(defn draft-observation-post
  "A dry-run observation post over yakushi's own domain (Religious-corp first-party pharmaceutical R&D actor — Wave 1 reference = OTC 抗アレルギー点眼薬 triplet (sodium cromoglicate, naphazoline HCl,…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "yakushi R0: live social posting is Council Lv6+ + operator + "
