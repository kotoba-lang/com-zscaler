(ns himawari.methods.social
  "social.cljc — 向日葵 — Solar PV Manufacturing DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from himawari's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "向日葵 — Solar PV Manufacturing が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over himawari's own domain (Solar-grade crystalline-silicon PV module manufacturing Tier-B actor (polysilicon feedstock QA → ingot/wafer → cell process → module…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "himawari R0: live social posting is Council Lv6+ + operator + "
