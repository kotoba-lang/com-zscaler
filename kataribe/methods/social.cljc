(ns kataribe.methods.social
  "social.cljc — 語部 — Press + Publishing + Translation DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from kataribe's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "語部 — Press + Publishing + Translation が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over kataribe's own domain (Non-profit religious-corp press + publishing + translation substrate. Community journalism + doctrine commentary + multilingual translation…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "kataribe R0: live social posting is Council Lv6+ + operator + "
