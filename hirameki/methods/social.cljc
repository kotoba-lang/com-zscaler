(ns hirameki.methods.social
  "social.cljc — hirameki DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from hirameki's own
...
(defn draft-observation-post
  "A dry-run observation post over hirameki's own domain (World PUBLIC-PATENT KG-mirror observatory — the all-technology generalization of tokigusuri 時薬 (the pharma patent-cliff subset). Ingests…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "hirameki R0: live social posting is Council Lv6+ + operator + "
