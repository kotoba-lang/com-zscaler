#!/usr/bin/env bb
;; tokigusuri 時薬 — autonomous heartbeat: load the pharma-barrier graph → append GROUND datoms.
(ns tokigusuri.methods.autorun
  "autorun.cljc — tokigusuri 時薬 deterministic heartbeat (ADR-2606171300, inochi/asobi pattern).

  One beat: load the pharma access-barrier seed graph, build the GROUND EAVT datoms (drug/
  exclusivity-barrier/holder/bearer NODES + their barrier :en/* 縁), and APPEND them as ONE
  content-addressed transaction to the append-only ACCESS-BARRIER LEDGER (kotoba.cljc).
  prev-cid chaining keeps the ledger tamper-evident + resume-safe.

  GROUND ONLY: access-barrier concentration / release readouts are DERIVED (computed on read,
  N1/G2) and are NEVER persisted. Deterministic by construction: the caller supplies tx-id +
  as-of (no wall clock); nodes emit in EDN-read order (node-ids) + edges content-stable →
...
#?(:clj
   (defn default-seed-path []
     (str (io/file here-dir "data" "seed-pharma-patent-graph.kotoba.edn"))))
...
(defn beat
  "Run one heartbeat. opts:
     ...
     :log-path access-barrier-ledger path (required)
...
#?(:clj
   (defn -main [& args]
     (let [log-path (or (first args)
                        (str (io/file here-dir "data" "persisted" "tokigusuri.access-barrier.kotoba.edn")))
           r (beat {:tx-id "tokigusuri-beat-manual" :as-of "manual" :log-path log-path})]
       (println (str "access-barrier ledger head=" (:head r)
                     " datoms=" (:count r)
                     " appended=" (:appended r)
