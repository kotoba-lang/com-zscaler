#!/usr/bin/env bb
;; tokigusuri 時薬 — pharma access-barrier graph DATOM-LOG writer (content-addressed commit-DAG).
(ns tokigusuri.methods.kotoba
  "kotoba.cljc — tokigusuri 時薬 append-only ACCESS-BARRIER LEDGER writer
  (ADR-2606171300, on ADR-2605262130 + ADR-2605312345; same content-addressed
  commit-DAG machinery as the inochi/asobi/hokorobi/tate/ugachi/busshi family — the
  datomic-isomorphic canonical-state layer tokigusuri previously only EMITTED
  (datom_emit) but never PERSISTED).

  Persists tokigusuri's GROUND datoms — the drug / exclusivity-barrier / holder / bearer
  NODES and the barrier :en/* 縁 (DISCLOSED Orange-Book/WHO-EML/MPP data, G1) — to a local
  append-only EDN log of content-addressed transactions. DERIVED state (access-barrier
  concentration / release readouts, the integral of incident 縁 computed on READ, N1/G2)
  is NEVER stored.
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; tokigusuri 時薬 — ACCESS-BARRIER LEDGER (append-only, content-addressed "
                        "EAVT commit-DAG of the drug/exclusivity-barrier/holder/bearer graph). Generated; "
                        "DO NOT hand-edit. A RELEASE map (the patent cliff is a clock), never a patent-busting "
                        "or per-company verdict. ADR-2606171300 / ADR-2605312345.\n")))
