(ns xroad.main
  "Kotodama WASM entrypoint for the Xroad clean-room actor (L4) — Clojure port.
  ...
  facts (`xroad.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"xroad.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "xroad-compat" :tier tier :entities entities} 200])
