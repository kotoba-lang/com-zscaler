(ns tsumugi.cells.social-post.state-machine
  "Phase state machine for the 紡ぎ (tsumugi) social_post cell — the publication membrane
  that lets the actor self-publish its HISTORY (取-concentration findings over public
  power-entities + the RELEASE routing) to the mesh/AT-proto WITHOUT a server-held key.
  ADR-2606272355 (actor self-publication seed).

  1:1 port of the danjo membrane (danjo.cells.social-post.state-machine), itself a mirror
  of the constellation membrane (keizu.cells.social-post / kosatsu social_post), adapted to
  tsumugi's power-graph / map-not-target posture. A record (a 取-concentration finding, a
  release-routing line, or a 産官学報 cluster note) enters; it is DRAFTED into a dry-run post
  ONLY if:

    G5(tsumugi) — ≥2 public primary-source / authoritative citations are present (sourcing
                honesty; no fabricated coverage);
    G2(tsumugi) — the post is a non-adjudicating, edge-primary mirror (isMirror), opening with
                the 取-concentration release disclaimer; it narrates the integral of incident
                縁, never a verdict, never a target-list, never a per-soul score;
    no-server-key — server-held-key is false (the actor self-custodies its own key in its
                kotoba-mesh WASM runtime and signs there; the server never does,
                ADR-2605231525);
    R0-gate(G7) — the status is dry-run (a 'published' request REFUSES — live publication
                needs Council Lv6+ + operator + a member/actor signature, §1.12 / G11).

  Self-contained. Stdlib only. Deterministic — the seed grows on the mesh, not here."
  (:require [clojure.string :as str]))

(def disclaimer
  "【観測ミラー / 取-concentration release map — NOT a target-list, 非断定・person-excluded】")

(def phase-init "init")
(def phase-drafted "drafted")
(def phase-refused "refused")

(def state-defaults
  {"phase"            phase-init
   "subject"          ""
   "sources"          []
   "requested_status" "dry-run"
   "server_held_key"  false
   "payload"          {}
   "refusal"          ""})

(defn- cell-state [state]
  (merge state-defaults (get state "cell_state" {})))

(defn- lstrip-colon [s]
  (str/replace (str s) #"^:+" ""))

(defn transition-to-drafted
  "Drive one record toward a dry-run post payload, or refuse with the failed invariant.
  Pure: (state) -> {\"cell_state\" {…}}."
  [state]
  (let [cs0 (cell-state state)
        cs  (assoc cs0
                   "subject"          (get state "subject" (get cs0 "subject"))
                   "sources"          (get state "sources" (get cs0 "sources"))
                   "requested_status" (lstrip-colon (get state "requested_status" (get cs0 "requested_status")))
                   "server_held_key"  (boolean (get state "server_held_key" (get cs0 "server_held_key"))))
        refuse (fn [msg]
                 {"cell_state" (assoc cs "refusal" msg "phase" phase-refused)})]
    (cond
      (< (count (get cs "sources")) 2)
      (refuse "G5(tsumugi): a post needs ≥2 public primary-source/authoritative citations")

      (get cs "server_held_key")
      (refuse "no-server-key: server-held-key must be false; the actor self-signs in its mesh runtime (ADR-2605231525)")

      (not= (get cs "requested_status") "dry-run")
      (refuse "R0-gate(G7): only dry-run posts; live publication is Council Lv6+ + operator + member/actor-signature gated (§1.12/G11)")

      :else
      (let [payload {":post/subject" (get cs "subject")
                     ":post/body" (str disclaimer " " (get cs "subject"))
                     ":post/status" ":dry-run"
                     ":post/is-mirror" true
                     ":post/non-adjudicating-notice" true
                     ":post/server-held-key" false
                     ":post/sources" (get cs "sources")}]
        {"cell_state" (assoc cs "payload" payload "refusal" "" "phase" phase-drafted)}))))
