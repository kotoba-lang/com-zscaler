(ns matsurigoto.methods.edn
  "Minimal EDN reader (subset: [] {} :kw \"str\" num bool nil). ADR-2606062300.
  ...
  STRINGS, NOT as Clojure keywords. standard.py / the charter-invariant tests key
  every COFOG / service / profile record on string keys (\":egov.service/id\",
  \":cofog/code\", \":country-profile/iso3\", …) and the Python `:`-strings stay
  strings (root CLAUDE.md convention) — so the loader must yield the same string
  shape the Python `load_edn` does, byte-for-byte, or the standard validator/coverage
  would key on the wrong thing.

  Matches the ake/keizu/shionome reference (`*.methods._edn`) — matsurigoto's `_edn.py`
...
(defn- next-tok! [state]
  (let [ts @state]
    (when (empty? ts)
      (throw (ex-info "matsurigoto.methods.edn: unexpected end of input" {})))
    (reset! state (rest ts))
    (first ts)))
