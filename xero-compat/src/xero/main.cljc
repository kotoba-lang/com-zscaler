(ns xero.main
  "Kotodama WASM entrypoint for the Xero clean-room actor (L5) — Clojure port.
  ...
  facts (`xero.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Invoice"        :plural "invoices"        :id-prefix "xero_inv"
    :fields [:invoiceNumber :type :status :total :amountDue]   :required [:invoiceNumber :type]
    :coerce {:total :float :amountDue :float}   :refs {}}
   {:entity "Contact"        :plural "contacts"        :id-prefix "xero_con"
    :fields [:name :emailAddress :contactStatus :taxNumber :isSupplier :isCustomer]   :required [:name :emailAddress]
    :coerce {:isSupplier :bool :isCustomer :bool}   :refs {}}
   {:entity "Account"        :plural "accounts"        :id-prefix "xero_acc"
    :fields [:code :name :type :status :bankAccountNumber]   :required [:code :name]
    :coerce {}   :refs {}}
   {:entity "Payment"        :plural "payments"        :id-prefix "xero_pay"
    :fields [:amount :status :paymentType :isReconciled]   :required [:amount :status]
    :coerce {:amount :float :isReconciled :bool}   :refs {}}
   {:entity "BankTransaction" :plural "banktransactions" :id-prefix "xero_ban"
    :fields [:type :status :total :reference :isReconciled]   :required [:type :status]
    :coerce {:total :float :isReconciled :bool}   :refs {}}
   {:entity "CreditNote"     :plural "creditnotes"     :id-prefix "xero_cre"
    :fields [:creditNoteNumber :type :status :total]   :required [:creditNoteNumber :type]
...
(defn emit-facts
  "EAVT facts for one record: {\"xero.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "xero-compat" :tier tier :entities entities} 200])
