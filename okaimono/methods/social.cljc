(ns okaimono.methods.social
  "social.cljc — okaimono DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from okaimono's own
...
(defn draft-observation-post
  "A dry-run observation post over okaimono's own domain (Amazon-inverse provisioning commons — Ring 0 commons-first (borrow/repair/secondhand/surplus) → Ring 1 internal SBT↔SBT economy over…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "okaimono R0: live social posting is Council Lv6+ + operator + "
