(ns suimin.methods.social
  "social.cljc — 睡眠 (suimin) DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from suimin's own
...
(defn draft-observation-post
  "A dry-run observation post over suimin's own domain (Religious-corp first-party sleep-disorder treatment-EVIDENCE research + synthesis actor — sibling of mitate (diagnostic routing). suimin…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "suimin R0: live social posting is Council Lv6+ + operator + "
