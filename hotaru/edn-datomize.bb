#!/usr/bin/env bb
;; 20-actors/hotaru/edn-datomize.bb — EDN → Datomic/Datascript tx-data 変換ツール。
;; Ported/adapted from 20-actors/hikari/edn-datomize.bb (same repo, Phase 4 fan-out,
;; itself adapted from 20-actors/makura/edn-datomize.bb + manifest/edn-datomize.bb).
;; schema.edn accumulates at 20-actors/hotaru/schema.edn (there is no manifest/ dir
;; at this level of etzhayyim/root).
...
;;            promote bare keys with ns-name (data/seed-iii-v-substrate.kotoba.edn) ----------
...
(defn -main [& args]
  (let [[mode a b] args]
    (case mode
      ...
      (do (println "usage: bb edn-datomize.bb [wrap-map-force-ns <path> <ns> | wrap-vector-keep-ns <path> <ns>]")
          (System/exit 1)))))
