 ;; APQC PCF Coordinator cell
;; kotoba-clj port of the open-apqc coordinator
...
(defn lookup [state]
  (let [code (map-get state "code")
        objs (kqe-get-objects "open-apqc" code "apqc.process/name")]
    (if (= (kqe-count objs) 1)
...
(defn summarize [state]
  (let [code (map-get state "code")]
    (map-assoc! state "result" (llm-infer "apqc-summarizer" code))))
...
(defn coverage [state]
  (map-assoc! state "result"
    (bytes-finish (cbor-enc-uint! (bytes-alloc 16)
                    (kqe-count (kqe-query "apqc.process/name"))))))

(defn coverage-ratio [state]
  (let [names (kqe-count (kqe-query "apqc.process/name"))
        parents (kqe-count (kqe-query "apqc.process/parent"))
...
(defn parent-of [state]
  (let [objs (kqe-get-objects "open-apqc" (map-get state "code") "apqc.process/parent")]
    (if (= (kqe-count objs) 1)
      (map-assoc! state "result" (kqe-obj-nth objs 0))
...
(defn child-count [state]
  (let [code (map-get state "code")
        ...
        qs (kqe-query "apqc.process/parent")]
    (map-assoc! state "result"
