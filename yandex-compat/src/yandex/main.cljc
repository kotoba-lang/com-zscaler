(ns yandex.main
  "Kotodama WASM entrypoint for the Yandex clean-room actor (L4) — Clojure port.
  ...
  facts (`yandex.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "yandex")
(def tier "L4")
...
(def entity-specs
  [{:entity "Order"      :plural "orders"      :id-prefix "yandex_ord"
   ...
   {:entity "Tracking"   :plural "trackings"   :id-prefix "yandex_tra"
...
(defn emit-facts
  "EAVT facts for one record: {\"yandex.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "yandex-compat" :tier tier :entities entities} 200])
...
(defn start! [] :yandex-compat/ready)
