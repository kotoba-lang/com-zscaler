(ns hikari.methods.social
  "social.cljc — 光 — Energy DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from hikari's own
...
(defn draft-observation-post
  "A dry-run observation post over hikari's own domain (Distributed renewable energy Tier-B actor (solar PV + small wind ≤100 kW + geothermal micro ≤500 kW + battery storage + grid-edge…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "hikari R0: live social posting is Council Lv6+ + operator + "
