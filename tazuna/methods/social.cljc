(ns tazuna.methods.social
  "social.cljc — tazuna DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from tazuna's own
...
(defn draft-observation-post
  "A dry-run observation post over tazuna's own domain (Clean-room remote-robotics fleet operation + teleoperation + learning-from-demonstration (手綱 = the reins: hold them to guide a robot that…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "tazuna R0: live social posting is Council Lv6+ + operator + "
