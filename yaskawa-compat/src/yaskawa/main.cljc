(ns yaskawa.main
  "Kotodama WASM entrypoint for the Yaskawa clean-room actor (L4) — Clojure port.
  ...
  facts (`yaskawa.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "yaskawa")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"yaskawa.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "yaskawa-compat" :tier tier :entities entities} 200])
...
(defn start! [] :yaskawa-compat/ready)
