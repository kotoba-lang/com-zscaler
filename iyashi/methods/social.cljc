(ns iyashi.methods.social
  "social.cljc — 癒 — Clinical Care Provider DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from iyashi's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "癒 — Clinical Care Provider が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over iyashi's own domain (Non-profit religious-corp clinical care provider substrate. Community-clinic-model primary care + chronic followup + vaccination + acute…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "iyashi R0: live social posting is Council Lv6+ + operator + "
