(ns zendesksell.main
  "Kotodama WASM entrypoint for the Zendesksell clean-room actor (L4) — Clojure port.
  ...
  facts (`zendesksell.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "zendesksell")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"zendesksell.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "zendesksell-compat" :tier tier :entities entities} 200])
...
(defn start! [] :zendesksell-compat/ready)
