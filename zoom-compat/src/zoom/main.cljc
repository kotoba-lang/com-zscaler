(ns zoom.main
  "Kotodama WASM entrypoint for the Zoom clean-room actor (L4) — Clojure port.
  ...
  facts (`zoom.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Meeting"   :plural "meetings"   :id-prefix "zoom_mee"
    :fields [:topic :type :status :duration :timezone :hostId :startTime] :required [:topic :type]
    :coerce {:type :int}                              :refs {}}
   {:entity "Webinar"   :plural "webinars"   :id-prefix "zoom_web"
    :fields [:topic :type :duration :hostId]         :required [:topic :type]
    :coerce {:type :int}                              :refs {}}
   {:entity "Recording" :plural "recordings" :id-prefix "zoom_rec"
    :fields [:meetingId :recordingType :duration :fileSize :status] :required [:recordingType :duration]
    :coerce {:duration :int :fileSize :int}           :refs {:meetingId "Meeting"}}
   {:entity "Registrant" :plural "registrants" :id-prefix "zoom_reg"
    :fields [:email :firstName :lastName :status]     :required [:email :firstName]
    :coerce {}                                         :refs {}}
   {:entity "User"     :plural "users"     :id-prefix "zoom_use"
    :fields [:email :firstName :lastName :type :status] :required [:email :firstName]
    :coerce {:type :int}                               :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"zoom.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "zoom-compat" :tier tier :entities entities} 200])
