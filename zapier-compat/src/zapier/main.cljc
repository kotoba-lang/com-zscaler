(ns zapier.main
  "Kotodama WASM entrypoint for the Zapier clean-room actor (L4) — Clojure port.
  ...
  facts (`zapier.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "zapier")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"zapier.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "zapier-compat" :tier tier :entities entities} 200])
...
(defn start! [] :zapier-compat/ready)
