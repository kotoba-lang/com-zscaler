(ns organizer.methods.social
  "social.cljc — organizer DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from organizer's own
...
(defn draft-observation-post
  "A dry-run observation post over organizer's own domain (Google-Drive-inverse auto-organize file commons — upload → AI auto-classifies + groups into collections, FREE with no storage tiers and NO…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "organizer R0: live social posting is Council Lv6+ + operator + "
