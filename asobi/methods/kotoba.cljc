#!/usr/bin/env bb
;; asobi 遊び — freed-time/play-graph DATOM-LOG writer (content-addressed commit-DAG).
(ns asobi.methods.kotoba
  "kotoba.cljc — asobi 遊び append-only PLAY-COMMONS LEDGER writer
  (ADR-2606073200, on ADR-2605262130 + ADR-2605312345; same content-addressed
  commit-DAG machinery as the inochi/tate/hinagata/ugachi/busshi family — the
  datomic-isomorphic canonical-state layer asobi previously only EMITTED
  (datom_emit) but never PERSISTED).

  Persists asobi's GROUND datoms — the work/event/venue/practice NODES and the
  access/enclosure :en/* 縁 (PUBLIC reference cultural-participation data, G1) — to a
  local append-only EDN log of content-addressed transactions. DERIVED state
  (participation-openness / enclosure-load / enclosure-imposed, the integral of
  incident 縁 computed on READ, N1/G2) is NEVER stored.
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; asobi 遊び — PLAY-COMMONS LEDGER (append-only, content-addressed EAVT "
                        "commit-DAG of the freed-time work/event/venue/access graph). Generated; "
                        "DO NOT hand-edit. An ACCESS map routed to OPENING, never an engagement "
                        "ranking. ADR-2606073200 / ADR-2605312345.\n")))
