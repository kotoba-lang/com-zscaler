(ns danjo.cells.social-post.state-machine
  "Phase state machine for the 弾正 (danjo) social_post cell — the publication membrane
  ...
  kosatsu social_post) adapted to danjo's accountability-mirror posture. A record
  (an oversight observation, a revenue-ledger line, or a procedure note) enters; it
  is DRAFTED into a dry-run post ONLY if:

    G5(danjo) — ≥2 public gov.dataset.* / primary-source citations are present;
    G4(danjo) — the post is a non-adjudicating mirror (isMirror), opening with the
                accountability disclaimer; it narrates facts, never a verdict;
...
(def disclaimer
  "【観測ミラー / accountability map — NOT the government, NOT a prosecutor, 非断定】")
...
(defn transition-to-drafted
  ...
  (let [cs0 (cell-state state)
    ...
    (cond
      ...
      (refuse "G5(danjo): a post needs ≥2 public gov.dataset.*/primary-source citations")
