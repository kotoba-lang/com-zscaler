(ns yokogawa-cx.main
  "Kotodama WASM entrypoint for the Yokogawa_cx clean-room actor (L4) — Clojure port.
  ...
  facts (`yokogawa_cx.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "yokogawa_cx")
(def tier "L4")
...
(def entity-specs
  [{:entity "Tag"         :plural "tags"         :id-prefix "yokogawa_tag"
   ...
   {:entity "ControlLoop" :plural "controlloops" :id-prefix "yokogawa_con"
    :fields [:name :setpoint :processValue :mode]           :required [:name :setpoint]
    :coerce {:setpoint :float :processValue :float}         :refs {}}
   {:entity "Batch"       :plural "batches"      :id-prefix "yokogawa_bat"
...
(defn emit-facts
  "EAVT facts for one record: {\"yokogawa_cx.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "yokogawa_cx-compat" :tier tier :entities entities} 200])
...
(defn start! [] :yokogawa_cx-compat/ready)
