(ns yokogawa-cx.main-test
  "Contract + behavioral test for the yokogawa-cx L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "yokogawa_cx-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'yokogawa-cx.main-test)]
