(ns x86-64-isa.main-test
  "Contract + behavioral test for the x86_64_isa-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "x86_64_isa-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'x86-64-isa.main-test)]
