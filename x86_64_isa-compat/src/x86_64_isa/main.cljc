(ns x86-64-isa.main
  "Kotodama WASM entrypoint for the x86_64_isa clean-room actor (L4) — Clojure port.
  ...
  facts (`x86_64_isa.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "x86_64_isa")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"x86_64_isa.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "x86_64_isa-compat" :tier tier :entities entities} 200])
...
(defn start! [] :x86_64_isa-compat/ready)
