(ns watatsuna.methods.social
  "social.cljc — 綿津綱 (わたつな) DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from watatsuna's own
...
(defn draft-observation-post
  "A dry-run observation post over watatsuna's own domain (World submarine-cable network knowledge graph — datafies cable SYSTEMS, LANDING STATIONS, SEGMENTS and FAULT bulletins into the kotoba…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "watatsuna R0: live social posting is Council Lv6+ + operator + "
