(ns suji.methods.social
  "social.cljc — suji DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from suji's own
...
(defn draft-observation-post
  "A dry-run observation post over suji's own domain (Musculoskeletal posture-load biomechanics simulator — the PHYSICS-SIMULATION sibling of kizashi 兆 (which SENSES the body) and upstream of…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "suji R0: live social posting is Council Lv6+ + operator + "
