(ns kafun.methods.social
  "social.cljc — kafun DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from kafun's own
...
(defn draft-observation-post
  "A dry-run observation post over kafun's own domain (花粉撲滅 remediation gate. Edge-primary pollen-source concentration (stand-area × emission-density × exposed-population-weight, on read) routed…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "kafun R0: live social posting is Council Lv6+ + operator + "
