#!/usr/bin/env bb
;; kafun 花粉 — remediation-ledger kotoba Datom-log writer (content-addressed).
(ns kafun.methods.kotoba
  "kotoba.cljc — kafun 花粉 append-only REMEDIATION LEDGER writer
  (ADR-2606211712, on ADR-2605262130 + ADR-2605312345; same content-addressed
  commit-DAG machinery as the ugachi/meisai/kakaku family).

  Persists the 花粉撲滅 remediation VERDICTS to a local append-only EDN log of
  content-addressed transactions — the 'remediation ledger'. Each tx CID =
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; kafun 花粉 — REMEDIATION LEDGER (append-only, content-addressed "
                        "EAVT commit-DAG of 花粉撲滅 remediation verdicts). Generated; DO NOT hand-edit. "
                        "A record of restoration DECISIONS, never a cut-list. ADR-2606211712.\n")))
