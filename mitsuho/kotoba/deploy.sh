#!/usr/bin/env bash
# mitsuho 瑞穂 — kotoba deploy
# ADR-2605261015 · migration plan Phase 3
#
# Ingests schema-shaped seed datoms (parcels, crop plans, harvests, food lots)
# into a running kotoba node and (optionally) builds the langgraph WASM actor.
# Writes to the canonical Datom journal require an authorized operator session
# token (no-server-key posture, G4). Without KOTOBA_TOKEN the ingest is a
# dry-run. Replaces the legacy graph/RisingWave path (G6).
#
# Usage:
...
GRAPH="${MITSUHO_GRAPH:-com.etzhayyim.mitsuho}"
ACTOR_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

echo "==> mitsuho kotoba deploy → ${KOTOBA_URL} (graph ${GRAPH})"
...
# Parcel + crop plan + harvest + food lot seed ingestion (representative R0)
echo "--> food & agriculture datoms ingest (parcel/* + crop-plan/* + harvest/* + food-lot/*)"
