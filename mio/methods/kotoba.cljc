#!/usr/bin/env bb
;; 澪 mio — verification-ledger kotoba Datom-log writer (content-addressed).
(ns mio.methods.kotoba
  "kotoba.cljc — 澪 mio append-only VERIFICATION LEDGER writer (on ADR-2605262130
  + ADR-2605312345; same content-addressed commit-DAG machinery as the
  busshi/ugachi/meisai family).

  Persists the Proof-of-Useful-Flow VERDICTS to a local append-only EDN log of
  content-addressed transactions — the org's verified-flow record. Each tx CID =
  'b' + sha256-hex over the canonical JSON {\"datoms\":[…],\"prev\":…} (keys stay
  ':…' STRINGS; EAVT = [op entity attr value], op :db/add only). prev-cid chaining
  makes the log tamper-evident (verify-chain). Deterministic: caller supplies
  tx-id + as-of (no wall clock) → resume-safe.

  No-server-key: this writer holds no key and performs NO network I/O — it only
  appends to a local file. The ledger is auditable (a record of VERIFIED ORDERED
  FLOW, never a market signal; reward issuance stays 1 SBT=1 vote + TitheRouter)."
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; 澪 mio — VERIFICATION LEDGER (append-only, content-addressed "
                        "EAVT commit-DAG of Proof-of-Useful-Flow verdicts). Generated; DO NOT "
                        "hand-edit. A record of VERIFIED ORDERED FLOW, never a market signal.\n")))
