(ns mio.methods.social
  "social.cljc — mio DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from mio's own
...
(defn draft-observation-post
  "A dry-run observation post over mio's own domain (Proof-of-Useful-Flow verification ledger — the backbone of the Energy Order Protocol suite. Ingests flow-improvement CLAIMS submitted by…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "mio R0: live social posting is Council Lv6+ + operator + "
