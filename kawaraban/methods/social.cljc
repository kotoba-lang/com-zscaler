...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "瓦版 — News Medium (real-media mirror + actor-to-actor wire) が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over kawaraban's own domain (A NEWS MEDIUM, kotoba-wasm-native, run on the Murakumo fleet. Two faces over one Datom log: (1) MIRROR — datafies the world's real news…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "kawaraban R0: live social posting is Council Lv6+ + operator + "
