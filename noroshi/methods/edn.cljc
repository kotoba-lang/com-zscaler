(ns noroshi.methods.edn
  "Minimal EDN reader (subset: [] {} :kw \"str\" num bool nil). ADR-2606051600.
  Clojure port of noroshi's `methods/_edn.py` (itself ported from watatsuna/tsumugi).

  The fidelity invariant this preserves: keywords are kept as their \":ns/name\"
  STRINGS, NOT as Clojure keywords. cable_endpoint.py keys every watatsuna seed
  record on string keys (\":cable/id\", \":station/chokepoint\", \":cable.seg/traverses\",
  ...
  or the offline join would key on the wrong thing.

  noroshi's `_edn.py` is the SIMPLE family (the `.replace`-based string unescape, NOT
  shionome's json.loads variant): used by cable_endpoint.py to read the watatsuna
  submarine-cable seed without a dependency, mirroring the watatsuna analyzer's own
  parser for parity.
...
(defn- next-tok! [state]
  (let [ts @state]
    (when (empty? ts)
      (throw (ex-info "noroshi.methods.edn: unexpected end of input" {})))
    (reset! state (rest ts))
    (first ts)))
