(ns talent.methods.social
  "social.cljc — talent DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from talent's own
...
(defn draft-observation-post
  "A dry-run observation post over talent's own domain (Recruiter-DB-inverse cohort-first talent registry (ISCO-08, PII Tier 3). Profiles are SELF-SOVEREIGN only (caller DID = subject DID; no…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "talent R0: live social posting is Council Lv6+ + operator + "
