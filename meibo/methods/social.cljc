(ns meibo.methods.social
  "social.cljc — meibo DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from meibo's own
...
(defn draft-observation-post
  "A dry-run observation post over meibo's own domain (Verified legal-institution directory registry — the honest, lawful implementation of gftdcojp ADR-0016's never-built…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "meibo R0: live social posting is Council Lv6+ + operator + "
