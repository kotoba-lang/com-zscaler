(ns amime.methods.social
  "social.cljc — amime DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from amime's own
...
(defn draft-observation-post
  "A dry-run observation post over amime's own domain (The multi-site energy-FLOW NETWORK the Energy Order suite (mio/tawami/okibi/toi/yudane/hikari, ADR-2606211200) lacked. hikari 光 designs ONE…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "amime R0: live social posting is Council Lv6+ + operator + "
