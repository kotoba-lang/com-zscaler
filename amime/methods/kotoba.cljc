#!/usr/bin/env bb
;; amime 網目 — mesh-resilience ledger kotoba Datom-log writer (content-addressed).
(ns amime.methods.kotoba
  "kotoba.cljc — amime 網目 append-only MESH-RESILIENCE LEDGER writer (ADR-2606212020,
  on ADR-2605262130 + ADR-2605312345; the same content-addressed commit-DAG machinery as
  the ugachi/meisai/mio family).

  Persists each mesh solve (served fraction, transmission loss, critical link, per-site
  import-dependence) to a local append-only EDN log of content-addressed transactions.
  Each tx CID = 'b' + sha256-hex over the canonical JSON {\"datoms\":[…],\"prev\":…}; prev-cid
  chaining makes the log tamper-evident (verify-chain). Deterministic: caller supplies
  tx-id + as-of (no wall clock) → resume-safe.

  No-server-key: holds no key, NO network I/O — appends to a local file only. A resilience
  record, NEVER a target-list; a COMMONS mesh ledger, never a market book (no price/trade)."
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; amime 網目 — MESH-RESILIENCE LEDGER (append-only, content-addressed "
                        "EAVT commit-DAG of energy-mesh solves). Generated; DO NOT hand-edit. "
                        "A resilience record, never a target-list. ADR-2606212020.\n")))
