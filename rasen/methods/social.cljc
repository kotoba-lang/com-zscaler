(ns rasen.methods.social
  "social.cljc — rasen DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from rasen's own
...
(defn draft-observation-post
  "A dry-run observation post over rasen's own domain (Public-genetics (公開遺伝) Knowledge Graph mirror — weaves PUBLIC reference genetics (genes / variants / phenotypes / pathways +…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "rasen R0: live social posting is Council Lv6+ + operator + "
