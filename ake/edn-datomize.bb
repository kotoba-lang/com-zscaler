#!/usr/bin/env bb
;; edn-datomize.bb — EDN → Datomic/Datascript tx-data 変換ツール（ake 用に adapt）。
;; 出自: com-junkawasaki/root superproject の manifest/edn-datomize.cljs (Phase 1/2 実装)、
;; 同型 port は 20-actors/hataori/edn-datomize.bb (Phase 4, wrap-map-preserve variant) を参照。
;; schema-path はこのディレクトリ（20-actors/ake/）直下（repo-root に manifest/ 規約が無いため）。
;;
;; 使い方:
...
(def root (str (io/file (System/getProperty "user.dir") "20-actors" "ake")))
...
(defn pr-str-no-ns-map
  "pr-str with *print-namespace-maps* off. ake's own methods/_edn.cljc is a minimal
  hand-rolled EDN reader (bracket/atom tokenizer) that does NOT understand Clojure's
  #:ns{...} namespaced-map reader-tag shorthand (it would tokenize the leading
  '#:actor' as a bare atom and then parse the following '{...}' as a SEPARATE
  sibling form, corrupting the structure). Blobs must always print in the fully
  expanded {:actor/handle ... :actor/did ...} form so `ake.methods._edn/parse-edn`
  can re-read them byte-faithfully via `reconstitute`/`unblob`."
  [v]
