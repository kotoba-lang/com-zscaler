(ns yonyou.main
  "Kotodama WASM entrypoint for the Yonyou clean-room actor (L4) — Clojure port.
  ...
  facts (`yonyou.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "yonyou")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"yonyou.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "yonyou-compat" :tier tier :entities entities} 200])
...
(defn start! [] :yonyou-compat/ready)
