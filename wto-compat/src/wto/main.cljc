(ns wto.main
  "Kotodama WASM entrypoint for the Wto clean-room actor (L4) — Clojure port.
  ...
  facts (`wto.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"wto.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "wto-compat" :tier tier :entities entities} 200])
