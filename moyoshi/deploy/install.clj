#!/usr/bin/env bb
;; moyoshi 催し — install/uninstall the production heartbeat LaunchAgent (ADR-2606272100 R3).
;;   bb install.clj install    → render the plist from the template, load it, kickstart once
...
;; is ephemeral — re-run install from the merged checkout once moyoshi R3 lands on main.
(require '[babashka.fs :as fs]
         '[babashka.process :as p]
         '[clojure.string :as str])

(def label "com.etzhayyim.moyoshi.heartbeat")
(def here  (str (fs/parent (fs/absolutize *file*))))                          ; …/20-actors/moyoshi/deploy
...
(def log   (str home "/Library/Logs/moyoshi-heartbeat.log"))
...
(defn install []
  ...
  (let [rendered (-> (slurp (str (fs/path here "com.etzhayyim.moyoshi.heartbeat.plist.template")))
