(ns meyasu.methods.kotoba
  "meyasu 目安 — kotoba Datom-log writer (local, content-addressed). clj-native SSoT
  (ADR-2606142300 D1: new logic-core authored in Clojure) + ADR-2606073201 + ADR-2605262130.

  meyasu's `handle-persist` (py/agent.clj) builds the fused-card Datoms but, no-server-key, only
  RETURNS the tx. This is the autonomous-loop LOCAL write path the actor family uses (kanjō /
  shionome / uchiwake / meisai): a heartbeat appends content-addressed transactions to a local
  append-only EDN log with NO external I/O. Live-node push / atproto broadcast stay operator-gated.

  Constitution holds by construction (meyasu gates): only fused buyer-transparency + supply-
  resilience cards are representable; a point-asserted/speculative forecast never reaches a card
  (G2, refused at fuse); `:trade`/`:speculation` are unrepresentable (G1). EAVT = [op e a v], op is
  :db/add only (append-only). Deterministic: caller supplies tx-id + as-of (no wall clock).
...
(def log-default
  (-> (io/file *file*) .getParentFile .getParentFile (io/file "data" "meyasu.datoms.kotoba.edn") str))

(defn append-tx [tx log-path]
  (let [f (io/file log-path)]
    (when-let [p (.getParentFile f)] (.mkdirs p))
    (when-not (.exists f)
      (spit f (str ";; meyasu kotoba Datom log — append-only EAVT transactions (content-addressed "
                   "DAG). Fused buyer-transparency + supply-resilience cards; a 目安, never a trade "
                   "(G1). DO NOT hand-edit. ADR-2606073201.\n")))
