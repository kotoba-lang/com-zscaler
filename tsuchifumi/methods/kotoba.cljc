#!/usr/bin/env bb
;; tsuchifumi 土踏み — observation-ledger kotoba Datom-log writer (content-addressed).
(ns tsuchifumi.methods.kotoba
  "kotoba.cljc — tsuchifumi 土踏み append-only OBSERVATION LEDGER writer
  (ADR-2606212000, on ADR-2605262130 + ADR-2605312345; the same content-addressed
  commit-DAG machinery as the kafun/ugachi/busshi/meisai family).

  Persists the earthing-EMF relief VERDICTS + risk register to a local append-only
  EDN log of content-addressed transactions. Each tx CID = 'b' + sha256-hex over the
  canonical JSON {\"datoms\":[…],\"prev\":…} (keys stay ':…' STRINGS; EAVT =
  [op entity attr value], op :db/add only). prev-cid chaining makes the log
  tamper-evident (verify-chain). Deterministic: caller supplies tx-id + as-of (no
  wall clock) → resume-safe.
  ...
  (a relief record, NEVER a harm/target list, NEVER person-level — G1/G3)."
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; tsuchifumi 土踏み — OBSERVATION LEDGER (append-only, content-addressed "
                        "EAVT commit-DAG of earthing-EMF relief verdicts + risk register). Generated; "
                        "DO NOT hand-edit. A relief record, NEVER a harm/target list, NEVER person-level. "
                        "ADR-2606212000.\n")))
