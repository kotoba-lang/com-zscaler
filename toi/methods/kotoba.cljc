#!/usr/bin/env bb
;; 樋 toi — routing-ledger kotoba Datom-log writer (content-addressed).
(ns toi.methods.kotoba
  "kotoba.cljc — 樋 toi append-only COMPUTE ROUTING LEDGER writer (on ADR-2605262130
  + ADR-2605312345; same content-addressed commit-DAG machinery as the mio/okibi
  family). Persists routings to a local append-only EDN log of content-addressed
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; 樋 toi — COMPUTE ROUTING LEDGER (append-only, content-addressed "
                        "EAVT commit-DAG of compute routings). Generated; DO NOT hand-edit. "
                        "A routing map, never a forced dispatch.\n")))
