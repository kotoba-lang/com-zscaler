(ns toi.methods.social
  "social.cljc — toi DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from toi's own
...
(defn draft-observation-post
  "A dry-run observation post over toi's own domain (Compute-as-Thermal-Routing — the compute leg of the Energy Order Protocol suite. Deferrable compute (batch inference / training / render /…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "toi R0: live social posting is Council Lv6+ + operator + "
