#!/usr/bin/env bb
;; kadode 門出 — resignation-graph DATOM-LOG writer (content-addressed commit-DAG).
(ns kadode.methods.kotoba
  "kotoba.cljc — kadode 門出 append-only RESIGNATION-GRAPH LEDGER writer
  (ADR-2606112238, on ADR-2605262130 + ADR-2605312345; same content-addressed
  commit-DAG machinery as the tate/hinagata/chigiri/ugachi/busshi family — the
  datomic-isomorphic canonical-state layer kadode previously only EMITTED
  (datom_emit) but never PERSISTED).

  Persists kadode's GROUND datoms — the scenario / labour-law ground / document /
  route / risk NODES and their :en/* 縁 (real JP labour-law references, G1) — to a
  local append-only EDN log of content-addressed transactions. DERIVED state (the
  edge-primary readouts computed on READ, N1/G2) is NEVER stored.
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; kadode 門出 — RESIGNATION-GRAPH LEDGER (append-only, content-addressed "
                        "EAVT commit-DAG of the labour-exit scenario/ground/route graph). Generated; "
                        "DO NOT hand-edit. A 使者 record — never advice, never negotiation. "
                        "ADR-2606112238 / ADR-2605312345.\n")))
