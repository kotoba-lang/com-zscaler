(ns kadode.methods.social
  "social.cljc — kadode DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from kadode's own
...
(defn draft-observation-post
  "A dry-run observation post over kadode's own domain (Labour-resignation concierge + 使者 (messenger) — helps a worker resign with dignity (門出 = setting out on a new path), grounded in actual…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "kadode R0: live social posting is Council Lv6+ + operator + "
