(ns yotpo.main
  "Kotodama WASM entrypoint for the Yotpo clean-room actor (L4) — Clojure port.
  ...
  facts (`yotpo.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"yotpo.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "yotpo-compat" :tier tier :entities entities} 200])
