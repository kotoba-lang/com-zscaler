(ns zozo.main
  "Kotodama WASM entrypoint for the Zozo clean-room actor (L4) — Clojure port.
  ...
  facts (`zozo.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Product"    :plural "products"    :id-prefix "zozo_pro"
   ...
   {:entity "Customer"   :plural "customers"   :id-prefix "zozo_cus"
    :fields [:email :firstName :lastName :phone] :required [:email :firstName]
    :coerce {}                       :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "zozo_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"zozo.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "zozo-compat" :tier tier :entities entities} 200])
