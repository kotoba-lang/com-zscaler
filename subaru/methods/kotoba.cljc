#!/usr/bin/env bb
;; subaru 昴 — constellation graph DATOM-LOG writer (content-addressed commit-DAG).
(ns subaru.methods.kotoba
  "kotoba.cljc — subaru 昴 append-only CONSTELLATION LEDGER writer
  (ADR-2606162355, on ADR-2605262130 + ADR-2605312345; same content-addressed
  commit-DAG machinery as the inochi/tate/rasen/aburi/ugachi/busshi family — the
  datomic-isomorphic canonical-state layer subaru previously only EMITTED (datom_emit)
  but never PERSISTED).

  Persists subaru's GROUND datoms — the constellation / bus / shell / link / ground-station /
  area / entitlement / disposal NODES and their :en/* 縁 (PUBLIC open-bus/protocol facts, G4) —
  to a local append-only EDN log of content-addressed transactions. DERIVED state (coverage
  reach / min link margin / deorbit debt, computed on READ, N1) is NEVER stored.
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; subaru 昴 — CONSTELLATION LEDGER (append-only, content-addressed EAVT "
                        "commit-DAG of the constellation/bus/shell/link/area/disposal graph). Generated; "
                        "DO NOT hand-edit. A connectivity-COMMONS map, never a surveillance/targeting "
                        "platform (no DPI/user-location/ISR-relay). ADR-2606162355 / ADR-2605312345.\n")))
