(ns tsukuroi.methods.social
  "social.cljc — 繕い — Mend (authorized vulnerability remediation + patch proposal) DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from tsukuroi's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "繕い — Mend (authorized vulnerability remediation + patch proposal) が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over tsukuroi's own domain (Non-profit religious-corp authorized vulnerability-remediation + patch-proposal actor; the constructive sibling of akuma (悪魔,…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "tsukuroi R0: live social posting is Council Lv6+ + operator + "
