(ns credits.methods.social
  "social.cljc — Credits — Credit Ledger & Public Fund Routing DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from credits's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "Credits — Credit Ledger & Public Fund Routing が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over credits's own domain (yoro.etzhayyim.com human-participation credit ledger. Earn (HC tasks / Murakumo compute) -> Purchase (fixed 30% platform fee) -> Spend…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "credits R0: live social posting is Council Lv6+ + operator + "
