(ns funamori.methods.social
  "social.cljc — funamori DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from funamori's own
...
(defn draft-observation-post
  "A dry-run observation post over funamori's own domain (Marine-renewable salinity-gradient power (淡水化発電) — generating electricity from the Gibbs free energy released when fresh river water mixes…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "funamori R0: live social posting is Council Lv6+ + operator + "
