#!/usr/bin/env bash
# watatsumi 綿津見 — kotoba deploy
# ADR-2605252200 · migration plan Phase 3
#
# Ingests schema-shaped seed datoms (submersible hulls + systems + trials) into a
# running kotoba node and (optionally) builds the langgraph WASM actor (9-handler
...
GRAPH="${WATATSUMI_GRAPH:-com.etzhayyim.watatsumi}"
ACTOR_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

echo "==> watatsumi kotoba deploy → ${KOTOBA_URL} (graph ${GRAPH})"
...
echo "--> submersible manufacturing seed ingest (:material/* + :hull-ring/* + :section/* + :system/* datoms)"
