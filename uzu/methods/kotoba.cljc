#!/usr/bin/env bb
;; uzu 渦 — content-addressed append-only commit-DAG (the INFORMATION log).
(ns uzu.methods.kotoba
  "kotoba.cljc — uzu 渦 append-only INFORMATION log writer (content-addressed)
  (ADR-2606211500, on ADR-2605262130 + ADR-2605312345; same commit-DAG machinery as
  the ugachi/busshi/meisai family).

  This is the organism's information substrate — copyable, append-only, NON-conserved
  (the opposite of the energy ledger, which is conserved and depletes). Each tx CID =
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; uzu 渦 — INFORMATION LOG (append-only, content-addressed EAVT "
                        "commit-DAG of organism beats). Generated; DO NOT hand-edit. "
                        "Copyable + non-conserved (the energy ledger is the conserved twin). "
                        "ADR-2606211500.\n")))
