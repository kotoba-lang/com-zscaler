(ns uzu.methods.social
  "social.cljc — uzu DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from uzu's own
...
(defn draft-observation-post
  "A dry-run observation post over uzu's own domain (An artificial organism built on the information-energy coupled view: a Markov-blanketed active-inference agent that draws finite free…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "uzu R0: live social posting is Council Lv6+ + operator + "
