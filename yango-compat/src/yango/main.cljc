(ns yango.main
  "Kotodama WASM entrypoint for the Yango clean-room actor (L4) — Clojure port.
  ...
  facts (`yango.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Driver"   :plural "drivers"   :id-prefix "yango_dri"
   ...
   {:entity "Rating"   :plural "ratings"   :id-prefix "yango_rat"
...
(defn emit-facts
  "EAVT facts for one record: {\"yango.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "yango-compat" :tier tier :entities entities} 200])
