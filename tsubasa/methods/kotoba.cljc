#!/usr/bin/env bb
;; tsubasa 翼 — fare-observation kotoba Datom-log writer (content-addressed).
(ns tsubasa.methods.kotoba
  "kotoba.cljc — tsubasa 翼 append-only FARE-OBSERVATION LEDGER writer
  (ADR-2606072800, on ADR-2605262130 + ADR-2605312345; same content-addressed
  commit-DAG machinery as the busshi/meisai/kakaku family).

  Persists the route-competition + fare observations to a local append-only EDN log
  of content-addressed transactions. Each tx CID = 'b' + sha256-hex over the
  canonical JSON {\"datoms\":[…],\"prev\":…} (keys stay ':…' STRINGS;
  EAVT = [op entity attr value], op :db/add only). prev-cid chaining makes the log
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; tsubasa 翼 — FARE-OBSERVATION LEDGER (append-only, content-addressed "
                        "EAVT commit-DAG of route-competition + fare observations). Generated; "
                        "DO NOT hand-edit. An honest fare/competition record, never a target-list "
                        "and never a paid ranking. ADR-2606072800.\n")))
