(ns shidemori.methods.social
  "social.cljc — 死出守 — Memorial + Cemetery DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from shidemori's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "死出守 — Memorial + Cemetery が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over shidemori's own domain (Non-profit religious-corp memorial + cemetery substrate. FINAL gap-closure actor (gap audit row 10; completes 10-actor 30min-loop wave).…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "shidemori R0: live social posting is Council Lv6+ + operator + "
