#!/usr/bin/env bb
;; busshi 物資 — stewardship-ledger kotoba Datom-log writer (content-addressed).
(ns busshi.methods.kotoba
  "kotoba.cljc — busshi 物資 append-only STEWARDSHIP LEDGER writer
  (ADR-2606171000, on ADR-2605262130 + ADR-2605312345; same content-addressed
  commit-DAG machinery as the meisai/kakaku family).

  Persists the resilience OBSERVATIONS to a local append-only EDN log of
  content-addressed transactions — the observatory's resilience observations.
  ...
  appends to a local file. Observations are :representative at R0; the ledger is auditable
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; busshi 物資 — OBSERVATION LEDGER (append-only, content-addressed "
                        "EAVT commit-DAG of resilience observations). Generated; DO NOT hand-edit. "
                        "A record of concentration OBSERVATIONS, never a target-list. ADR-2606171000.\n")))
