(ns busshi.methods.social
  "social.cljc — busshi DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from busshi's own
...
(defn draft-observation-post
  "A dry-run observation post over busshi's own domain (World commodity & raw-materials KG-mirror observatory. Mirrors commodities/materials (precious metals 金銀PGM / base metals / rare-critical…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "busshi R0: live social posting is Council Lv6+ + operator + "
