(ns toritate.methods.social
  "social.cljc — 執帳 — Accounting + Audit DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from toritate's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "執帳 — Accounting + Audit が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over toritate's own domain (Non-profit religious-corp accounting + audit substrate. Aggregates on-chain TitheRouter + Public Fund Safe + Council Safe + Land Registry…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "toritate R0: live social posting is Council Lv6+ + operator + "
