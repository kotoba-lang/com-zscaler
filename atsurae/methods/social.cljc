(ns atsurae.methods.social
  "social.cljc — atsurae DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from atsurae's own
...
(defn draft-observation-post
  "A dry-run observation post over atsurae's own domain (The Product Line Engineering (PLE) feature-model engine the roster lacked. open-kyber 開 (ADR-2606037200) covers ERP and uchiwake 内訳…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "atsurae R0: live social posting is Council Lv6+ + operator + "
