#!/usr/bin/env bb
;; atsurae 誂え — product-line ledger kotoba Datom-log writer (content-addressed).
(ns atsurae.methods.kotoba
  "kotoba.cljc — atsurae 誂え append-only PRODUCT-LINE LEDGER writer (ADR-2606212010, on
  ADR-2605262130 + ADR-2605312345; the same content-addressed commit-DAG machinery as the
  ugachi/amime/mio family).

  Persists each product-line analysis (per-feature commonality, variant count, variation
  points) to a local append-only EDN log of content-addressed transactions. Each tx CID = 'b'
  + sha256-hex over the canonical JSON {\"datoms\":[…],\"prev\":…}; prev-cid chaining makes the
  log tamper-evident (verify-chain). Deterministic: caller supplies tx-id + as-of (no wall
  clock) → resume-safe. No-server-key: local file only, no network I/O."
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; atsurae 誂え — PRODUCT-LINE LEDGER (append-only, content-addressed "
                        "EAVT commit-DAG of feature-model analyses). Generated; DO NOT hand-edit. "
                        "A commons spec record, never a license key. ADR-2606212010.\n")))
